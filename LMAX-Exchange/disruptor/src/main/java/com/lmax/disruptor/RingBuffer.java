...
abstract class RingBufferFields<E> extends RingBufferPad
{
    ...
    // 数组的开始地址，这里其实是数组中真正有效数据的开始地址，是整个数组开始地址+BUFFER_PAD个引用的偏移量
    ...
    static
    {
        ...
        if (4 == scale)
        {
            // 2^REF_ELEMENT_SHIFT = 2^2 = 4
            REF_ELEMENT_SHIFT = 2;
        }
        else if (8 == scale)
        {
            // 2^REF_ELEMENT_SHIFT = 2^3 = 8
            REF_ELEMENT_SHIFT = 3;
        }
        else
        {
        ...
        // 填充数组的长度: 128 / 4 = 32 || 128 / 8 = 16
        BUFFER_PAD = 128 / scale;  // 缓存行填充
        // Including the buffer pad in the array base offset
        // 计算Buffer数组起始基址
        // 在entries数组前后填充32或16个对象引用，保证前后各占用128个字节的内存，
        // 这样对于缓存行大小为64或128字节的系统来说，可以避免其它变量与entries数组元素在一个缓存行内，避免伪共享问题。
        REF_ARRAY_BASE = UNSAFE.arrayBaseOffset(Object[].class) + 128;
    }

    private final long indexMask;
    // 保存了RingBuffer每个槽的Event对象。这个entries不会被修改。 引用不会被修改
    private final Object[] entries;
    protected final int bufferSize;
    // sequencer是SingleProducerSequencer或者MultiProducerSequencer的引用
        ...
        Sequencer sequencer)
    {
        ...
        // 总长度 = sequencer的队列长度+2*padding(作为字节填充,解决伪共享问题)
        // 保证前后各占用128个字节的内存
        this.entries = new Object[sequencer.getBufferSize() + 2 * BUFFER_PAD];
        fill(eventFactory);  // entries[BUFFER_PAD + i] = eventFactory.newInstance();
    }
    ...
    protected final E elementAt(long sequence)
    {
        ...
        // sequence & indexMask 相当于 sequence % bufferSize
        // (sequence & indexMask) << REF_ELEMENT_SHIFT 相当于折算成一个对象
