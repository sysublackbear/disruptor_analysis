 ...
 * 3.BatchEventProcessor既是事件处理器，也是消费者。WorkProcessor是事件处理器，但不是消费者，WorkPool才是消费者。
 * 4.管理消费者，其实也是管理消费者们的所有事件处理器
 ...
 * 主要事件循环，处理Disruptor中的Event，并且拥有消费者的Sequence。它有一个实现类是BatchEventProcessor，包含了eventloop有效的实现，并且将回调
 * 到一个EventHandler接口的实现对象
 */
public interface EventProcessor extends Runnable
{
    /**
     * 获取事件处理器的Sequence(进度),每一个事件处理器有自己独立的Sequence
